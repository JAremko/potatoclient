...
(defn- md-header
  ...
  {:malli/schema [:=> [:cat :pos-int :string] :string]}
  [level text]
  (str (str/join (repeat level "#")) " " text))

(defn- md-code-block
  "Create a Markdown code block with optional language."
  {:malli/schema [:function [:=> [:cat :string] :string] [:=> [:cat :string [:maybe :string]] :string]]}
  ([code]
...
(defn- md-list-item
  ...
  {:malli/schema [:=> [:cat :string [:* :any]] :string]}
...
(defn- get-report-dir
  ...
  {:malli/schema [:=> [:cat] [:fn (partial instance? File)]]}
  []
  (io/file "reports"))
...
(defn- format-namespace-section
  ...
  {:malli/schema [:=> [:cat :symbol [:sequential :symbol]] :string]}
  [ns-sym functions]
