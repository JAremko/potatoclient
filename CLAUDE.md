...
│   └── kondo-gen/        # Clj-kondo config generator
...
**Location**: `tools/kondo-gen/`

Generates clj-kondo static analysis configurations from Malli function schemas:
- **Automatic discovery** of namespaces containing `:malli/schema` metadata
- **Fallback** to hardcoded list if discovery fails
- **Generates** type hints for all instrumented functions
- **Integrates** with CI/CD for static analysis without runtime

**Features:**
- Scans source files for Malli schemas (`:malli/schema`, `m/=>`, `mx/defn`)
- Collects function schemas from discovered namespaces
- Generates clj-kondo configs for IDE support and static analysis
- Provides discovery mode to inspect found namespaces

**Usage:**
```bash
# Generate configs (from project root)
make kondo-configs

# Show discovered namespaces
clojure -M:kondo-gen --discover

# Generate to custom directory
clojure -M:kondo-gen /path/to/output

# Run tests
cd tools/kondo-gen && make test
```

**Note:** In dev/nrepl mode, configs are automatically generated by `malli.dev/start!` when instrumentation is enabled. The tool is primarily for CI/CD and manual generation.
