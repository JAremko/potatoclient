...
(defn simple-inline
  "Simple function with inline malli metadata" {:malli/schema [:=> [:cat :string] :string]}
  [x]
  (str/upper-case x))
...
(defn- private-inline
  "Private function with inline metadata" {:malli/schema [:=> [:cat :int :int] :int]}
  [x y]
  (+ x y))

;; Case 4: Function without docstring but with metadata (needs moving)
(defn no-docstring {:malli/schema [:=> [:cat :string] :keyword]}
  [s]
  (keyword s))
...
(defn with-multiple-lambdas
  ...
  {:malli/schema [:=> [:cat 
                       [:fn (fn* [p1__1235#] (instance? JFrame p1__1235#))]
                       [:fn (fn* [p1__1236#] (instance? JPanel p1__1236#))]]
                  :nil]}
  [frame panel]
  (.add frame panel))
...
(defn with-error-message
  ...
  {:malli/schema [:=> [:cat [:fn {:error/message "must be a File"} 
                              (fn* [p1__1238#] (instance? File p1__1238#))]] 
                  :string]}
  [file]
  (.getName file))
...
(defn with-predicate-lambda
  ...
  {:malli/schema [:=> [:cat [:fn (fn* [p1__1239#] (string? p1__1239#))]] :keyword]}
  [s]
  (keyword s))
...
(defn with-custom-predicate
  ...
  {:malli/schema [:=> [:cat [:fn (fn* [p1__1240#] (instant? p1__1240#))]] :int]}
  [inst]
  (.getYear inst))
...
(defn with-nested-schema
  ...
  {:malli/schema [:=> [:cat [:map 
                              [:file [:fn (fn* [p1__1241#] (instance? File p1__1241#))]]
                              [:name :string]]]
                  :boolean]}
  [{:keys [file name]}]
  (= (.getName file) name))
