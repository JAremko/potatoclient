...
(defn- init-proto-explorer!
  ...
  []
  (try
    ;; Initialize specs directly
    (gen-specs/load-all-specs! "../../shared/specs/protobuf")
    (catch Exception e
...
(defn java-class-summary
  "Get human-readable Java class summary"
  [args]
  (init-proto-explorer!)
  (if-let [spec-name (first args)]
    (let [message-or-spec (parse-message-spec spec-name)
...
(defn dispatch-command
  ...
  (case command
    ...
    (output-edn {:error (str "Unknown command: " command)
                 :available-commands ["java-class" "java-fields" "java-builder" "java-summary"]})))
