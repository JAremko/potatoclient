...
;; Mapping of package names to protobuf class names
(def package-to-class
  {"cmd.CV" "JonSharedCmdCv"
   "cmd.OSD" "JonSharedCmdOsd"
   "cmd.RotaryPlatform" "JonSharedCmdRotary"
   "cmd.Lrf-calib" "JonSharedCmdLrfAlign"
...
;; Get the correct Java setter name
(defn get-setter-name [cmd-name pkg-name]
  (case pkg-name
    "cmd.CV" (case cmd-name
               "setCV" "setCv"
               (to-setter-method cmd-name))
    "cmd.OSD" (case cmd-name
                "setOSD" "setOsd"
                (to-setter-method cmd-name))
    "cmd.RotaryPlatform" (case cmd-name
                           "setRotaryPlatform" "setRotary"
                           (to-setter-method cmd-name))
    (to-setter-method cmd-name)))
...
(defn generate-builder [pkg-info]
  (let [pkg-name (:package pkg-info)
        ...
        cmd-pkg (if (str/starts-with? pkg-name "cmd.")
                  (subs pkg-name 4)
                  pkg-name)
        proto-class (get package-to-class pkg-name (str "JonSharedCmd" cmd-pkg))]
    (when (and (str/starts-with? pkg-name "cmd.")
               (seq commands))
      (println (str "\n=== " cmd-pkg " ==="))
      (println (str "Proto class: " proto-class))
      ...
      (str "package potatoclient.kotlin.transit.builders\n\n"
           ...
           "import cmd." cmd-pkg "." proto-class "\n"
           ...
           "                ." (get-setter-name (str "set" cmd-pkg) pkg-name) "(" (str/lower-case cmd-pkg) ")\n"
           "                .build()\n"
           "        }\n"
           "    }\n"
           "    \n"
           ...
           (str/join "\n"
                     (map (fn [cmd]
                            (let [;; Method name normalizes underscores to hyphens
                                  ...
                                  proto-class-name (to-proto-class (:schema-name cmd))]
                              (str "    private fun " method-name "(params: Map<*, *>): Result<" proto-class ".Root> = Result.success(\n"
                                   "        " proto-class ".Root.newBuilder()\n"
                                   "            ." setter-method "(" proto-class "." proto-class-name ".newBuilder().build())\n"
                                   "            .build()\n"
                                   "    )\n")))
                          commands))
           "}\n"))))
...
(doseq [pkg-name key-packages]
  (when-let [pkg (first (filter #(= pkg-name (:package %)) cmd-packages))]
    (let [builder-code (generate-builder pkg)]
      (when builder-code
        (let [output-file (str "generated-correct/" 
