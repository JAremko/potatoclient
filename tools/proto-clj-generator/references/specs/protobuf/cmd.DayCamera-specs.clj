(ns potatoclient.specs.cmd.DayCamera
...
(def move
  "Schema for move"
  [:map [:target-value [:and [:maybe :double] [:>= 0] [:<= 1]]]
   ...
   [:cmd
    [:oneof
     {:zoom [:map [:zoom :cmd/zoom]],
      :set-infra-red-filter [:map [:set-infra-red-filter :cmd/set-infra-red-filter]],
      ...
      :halt-all [:map [:halt-all :cmd/halt-all]],
      ...
      :focus [:map [:focus :cmd/focus]],
      :set-fx-mode [:map [:set-fx-mode :cmd/set-fx-mode]],
      :set-iris [:map [:set-iris :cmd/set-iris]],
      :refresh-fx-mode [:map [:refresh-fx-mode :cmd/refresh-fx-mode]],
      :set-auto-iris [:map [:set-auto-iris :cmd/set-auto-iris]],
   ...
   [:cmd
    [:oneof
     {:set-value [:map [:set-value :cmd/set-value]],
      ...
      :halt [:map [:halt :cmd/halt]],
      :offset [:map [:offset :cmd/offset]],
      :reset-focus [:map [:reset-focus :cmd/reset-focus]],
      :save-to-table-focus [:map [:save-to-table-focus :cmd/save-to-table-focus]]}]]])
