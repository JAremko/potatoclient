...
(def root
  ...
  [:map
   [:cmd
    [:oneof
     {:geodesic-mode-disable [:map [:geodesic-mode-disable :cmd/disable-geodesic-mode]],
      :start-all [:map [:start-all :cmd/start-a-ll]],
      :geodesic-mode-enable [:map [:geodesic-mode-enable :cmd/enable-geodesic-mode]],
      :localization [:map [:localization :cmd/set-localization]],
      :unmark-rec-important [:map [:unmark-rec-important :cmd/unmark-rec-important]],
      :stop-rec [:map [:stop-rec :cmd/stop-rec]],
      :reboot [:map [:reboot :cmd/reboot]],
      :start-rec [:map [:start-rec :cmd/start-rec]],
      :power-off [:map [:power-off :cmd/power-off]],
      :reset-configs [:map [:reset-configs :cmd/reset-configs]],
      :stop-all [:map [:stop-all :cmd/stop-a-ll]],
      :enter-transport [:map [:enter-transport :cmd/enter-transport]],
      :mark-rec-important [:map [:mark-rec-important :cmd/mark-rec-important]]}]]])
