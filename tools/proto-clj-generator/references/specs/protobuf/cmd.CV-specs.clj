...
(def root
  ...
  [:map
   [:cmd
    [:oneof
     {:vampire-mode-enable [:map [:vampire-mode-enable :cmd/vampire-mode-enable]],
      :vampire-mode-disable [:map [:vampire-mode-disable :cmd/vampire-mode-disable]],
      :dump-stop [:map [:dump-stop :cmd/dump-stop]],
      :stabilization-mode-disable [:map
                                   [:stabilization-mode-disable :cmd/stabilization-mode-disable]],
      :set-auto-focus [:map [:set-auto-focus :cmd/set-auto-focus]],
      :start-track-ndc [:map [:start-track-ndc :cmd/start-track-ndc]],
      :dump-start [:map [:dump-start :cmd/dump-start]],
      :stop-track [:map [:stop-track :cmd/stop-track]],
      :stabilization-mode-enable [:map
                                  [:stabilization-mode-enable :cmd/stabilization-mode-enable]]}]]])
...
(def set-auto-focus
  "Schema for set-auto-focus"
  [:map [:channel [:not [:enum [0]]]] [:value [:maybe :boolean]]])


(def start-track-ndc
  "Schema for start-track-ndc"
  [:map [:channel [:not [:enum [0]]]] [:x [:and [:maybe :double] [:>= -1] [:<= 1]]]
   [:y [:and [:maybe :double] [:>= -1] [:<= 1]]] [:frame-time [:maybe :int]]])
