...
### CUSTOM MALLI ONEOF SPEC
- **MUST USE**: `/home/jare/git/potatoclient/shared/specs/custom/potatoclient/specs/malli_oneof.clj`
- Our custom `:oneof` schema type is specifically designed for protobuf oneofs
- Generates proper validation that exactly one field must be set
- Includes generator support for property-based testing
- Already battle-tested in the main application
...
## 🎯 Core Insight: Binding and Spec Generation are Inseparable

The key realization from analyzing proto-explorer is that **binding generation and spec generation must happen together**. They share:
- The same type resolution logic
- The same constraint extraction
- The same IR structure
- The same validation requirements

Therefore, our implementation integrates both capabilities from the start.

## 🏗️ Implementation Strategy

### High-Level Phases

1. **Phase 0-1**: Foundation - Clean architecture, testing infrastructure, naming system
2. **Phase 2**: Enhanced Backend - Type system with constraint extraction
3. **Phase 3**: Unified IR - Specs and binding hints in one structure
4. **Phase 4**: Integrated Generation - Guardrails-enabled code with constraints
5. **Phase 5**: Validation Suite - Property-based testing with constraints

### Key Implementation Insights

1. **Oneof Handling**: Must use our custom Malli `:oneof` spec for proper validation
2. **Constraint Compilation**: Port proto-explorer's multimethod approach for clean constraint handling
3. **Type Registry**: Build global registry before generation to resolve cross-file references
4. **Metadata Pattern**: Use metadata to attach constraints without modifying base schemas
5. **Unified IR**: Single data structure contains both specs and binding generation info
...
- **MUST generate using our custom Malli `:oneof` spec**
- The custom spec ensures exactly one field is set
- Example usage:
  ```clojure
  [:oneof {:ping [:map [:ping :cmd/ping]]
...
Our approach recognizes that **binding generation and spec generation are two sides of the same coin**:

- **Consistency**: Specs and bindings always match
- **Completeness**: No validation rules lost between stages  
- **Efficiency**: Parse once, generate both
- **Maintainability**: Single source of truth in IR

The key insight from proto-explorer is that treating these as separate concerns leads to duplication and drift. By integrating them from the start, we build a more robust and maintainable generator.
...
## Phase 0: Foundation & Clean Architecture
**Goal**: Clean codebase with testing infrastructure and proper tooling

1. **Clean House**
   - [ ] Remove single-namespace mode completely
   - [ ] Remove non-guardrails templates
   - [ ] Delete all commented-out code
   - [ ] Remove unused functions

2. **Testing Infrastructure**
   - [ ] Set up property-based testing with test.check
   - [ ] Create Malli generators for proto types
   - [ ] Configure test coverage reporting
   - [ ] Set up integration test suite

3. **clj-kondo Configuration**
   - [ ] Copy `.clj-kondo/` config from main app
   - [ ] Configure guardrails macros support
   - [ ] Set up custom linters
   - [ ] Create lint reporting pipeline

**Validation Gates**: 
- Zero legacy code remains
- All existing tests pass
- clj-kondo configured and working
...
1. **Property-Based Testing with Constraints**
   - [ ] Generate test data using constraint-aware generators (e.g., `(gen/double* {:min -90.0 :max 90.0})`)
   - [ ] Port test patterns from `/home/jare/git/potatoclient/tools/proto-explorer/test/proto_explorer/constraints/compiler_test.clj`
   - [ ] Test all message types with valid data that passes buf.validate
   - [ ] Test constraint boundary conditions (gt vs gte, lt vs lte)
   - [ ] Test invalid data rejection matches buf.validate behavior

2. **Integration Testing**
   - [ ] Full roundtrip tests (proto → map → proto) with constraint validation
   - [ ] Verify generated data passes buf.validate checks
   - [ ] Cross-file type reference tests with constraints
   - [ ] Oneof handling verification with exactly-one-field validation
   - [ ] Performance benchmarks of constraint checking

3. **Validation Testing**
   - [ ] Test all buf.validate rules enforced in generated code
   - [ ] Test constraint compilation generates correct Malli schemas
   - [ ] Test guardrails catch both type errors and constraint violations
   - [ ] Test error messages include field context and constraint details
   - [ ] Test edge cases from JSON descriptors (CEL expressions, etc.)

**Validation Gates**:
- [ ] 100% of constraints have tests
- [ ] All roundtrips preserve data
