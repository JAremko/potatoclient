...
(def jon-gui-state-spec
  ...
  [:map [:protocol-version [:> 0]] [:system [:maybe :ser/jon-gui-data-system]]
...
(defn valid-jon-gui-data-time-timestamp?
  "Validate timestamp field of JonGuiDataTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :timestamp)
                               (drop 1 jon-gui-data-time-spec))))
              value))

(defn valid-jon-gui-data-time-manual-timestamp?
  "Validate manual-timestamp field of JonGuiDataTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :manual-timestamp)
                               (drop 1 jon-gui-data-time-spec))))
              value))

;; Validation helpers for JonGuiDataSystem
(defn valid-jon-gui-data-system-cpu-temperature?
  "Validate cpu-temperature field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cpu-temperature)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-gpu-temperature?
  "Validate gpu-temperature field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :gpu-temperature)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-gpu-load?
  "Validate gpu-load field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :gpu-load)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cpu-load?
  "Validate cpu-load field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cpu-load)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-power-consumption?
  "Validate power-consumption field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :power-consumption)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-loc?
  "Validate loc field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :loc)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cur-video-rec-dir-year?
  "Validate cur-video-rec-dir-year field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cur-video-rec-dir-year)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cur-video-rec-dir-month?
  "Validate cur-video-rec-dir-month field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cur-video-rec-dir-month)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cur-video-rec-dir-day?
  "Validate cur-video-rec-dir-day field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cur-video-rec-dir-day)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cur-video-rec-dir-hour?
  "Validate cur-video-rec-dir-hour field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cur-video-rec-dir-hour)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cur-video-rec-dir-minute?
  "Validate cur-video-rec-dir-minute field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cur-video-rec-dir-minute)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-cur-video-rec-dir-second?
  "Validate cur-video-rec-dir-second field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :cur-video-rec-dir-second)
                               (drop 1 jon-gui-data-system-spec))))
              value))

(defn valid-jon-gui-data-system-disk-space?
  "Validate disk-space field of JonGuiDataSystem - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :disk-space)
                               (drop 1 jon-gui-data-system-spec))))
              value))

;; Validation helpers for JonGuiDataLrf
(defn valid-jon-gui-data-lrf-measure-id?
  "Validate measure-id field of JonGuiDataLrf - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :measure-id)
                               (drop 1 jon-gui-data-lrf-spec))))
              value))

(defn valid-jon-gui-data-lrf-pointer-mode?
  "Validate pointer-mode field of JonGuiDataLrf - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :pointer-mode)
                               (drop 1 jon-gui-data-lrf-spec))))
              value))

;; Validation helpers for JonGuiDataTarget
(defn valid-jon-gui-data-target-timestamp?
  "Validate timestamp field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :timestamp)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-target-longitude?
  "Validate target-longitude field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-longitude)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-target-latitude?
  "Validate target-latitude field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-latitude)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-observer-longitude?
  "Validate observer-longitude field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :observer-longitude)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-observer-latitude?
  "Validate observer-latitude field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :observer-latitude)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-observer-azimuth?
  "Validate observer-azimuth field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :observer-azimuth)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-observer-elevation?
  "Validate observer-elevation field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :observer-elevation)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-observer-bank?
  "Validate observer-bank field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :observer-bank)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-distance-2d?
  "Validate distance-2d field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :distance-2d)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-distance-3b?
  "Validate distance-3b field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :distance-3b)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-observer-fix-type?
  "Validate observer-fix-type field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :observer-fix-type)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-session-id?
  "Validate session-id field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :session-id)
                               (drop 1 jon-gui-data-target-spec))))
              value))

(defn valid-jon-gui-data-target-target-id?
  "Validate target-id field of JonGuiDataTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-id)
                               (drop 1 jon-gui-data-target-spec))))
              value))
...
(defn valid-rgb-color-green?
  "Validate green field of RgbColor - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :green)
                               (drop 1 rgb-color-spec))))
              value))

(defn valid-rgb-color-blue?
  "Validate blue field of RgbColor - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :blue)
                               (drop 1 rgb-color-spec))))
              value))

;; Validation helpers for JonGuiDataGps
(defn valid-jon-gui-data-gps-longitude?
  "Validate longitude field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :longitude)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

(defn valid-jon-gui-data-gps-latitude?
  "Validate latitude field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :latitude)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

(defn valid-jon-gui-data-gps-altitude?
  "Validate altitude field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :altitude)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

(defn valid-jon-gui-data-gps-manual-longitude?
  "Validate manual-longitude field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :manual-longitude)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

(defn valid-jon-gui-data-gps-manual-latitude?
  "Validate manual-latitude field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :manual-latitude)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

(defn valid-jon-gui-data-gps-manual-altitude?
  "Validate manual-altitude field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :manual-altitude)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

(defn valid-jon-gui-data-gps-fix-type?
  "Validate fix-type field of JonGuiDataGps - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :fix-type)
                               (drop 1 jon-gui-data-gps-spec))))
              value))

;; Validation helpers for JonGuiDataCompass
(defn valid-jon-gui-data-compass-azimuth?
  "Validate azimuth field of JonGuiDataCompass - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :azimuth)
                               (drop 1 jon-gui-data-compass-spec))))
              value))

(defn valid-jon-gui-data-compass-elevation?
  "Validate elevation field of JonGuiDataCompass - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :elevation)
                               (drop 1 jon-gui-data-compass-spec))))
              value))

(defn valid-jon-gui-data-compass-bank?
  "Validate bank field of JonGuiDataCompass - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :bank)
                               (drop 1 jon-gui-data-compass-spec))))
              value))

(defn valid-jon-gui-data-compass-offset-azimuth?
  "Validate offset-azimuth field of JonGuiDataCompass - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :offset-azimuth)
                               (drop 1 jon-gui-data-compass-spec))))
              value))

(defn valid-jon-gui-data-compass-offset-elevation?
  "Validate offset-elevation field of JonGuiDataCompass - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :offset-elevation)
                               (drop 1 jon-gui-data-compass-spec))))
              value))

(defn valid-jon-gui-data-compass-magnetic-declination?
  "Validate magnetic-declination field of JonGuiDataCompass - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :magnetic-declination)
                               (drop 1 jon-gui-data-compass-spec))))
              value))

;; Validation helpers for JonGuiDataCompassCalibration
(defn valid-jon-gui-data-compass-calibration-stage?
  "Validate stage field of JonGuiDataCompassCalibration - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :stage)
                               (drop 1 jon-gui-data-compass-calibration-spec))))
              value))

(defn valid-jon-gui-data-compass-calibration-final-stage?
  "Validate final-stage field of JonGuiDataCompassCalibration - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :final-stage)
                               (drop 1 jon-gui-data-compass-calibration-spec))))
              value))

(defn valid-jon-gui-data-compass-calibration-target-azimuth?
  "Validate target-azimuth field of JonGuiDataCompassCalibration - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-azimuth)
                               (drop 1 jon-gui-data-compass-calibration-spec))))
              value))

(defn valid-jon-gui-data-compass-calibration-target-elevation?
  "Validate target-elevation field of JonGuiDataCompassCalibration - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-elevation)
                               (drop 1 jon-gui-data-compass-calibration-spec))))
              value))

(defn valid-jon-gui-data-compass-calibration-target-bank?
  "Validate target-bank field of JonGuiDataCompassCalibration - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-bank)
                               (drop 1 jon-gui-data-compass-calibration-spec))))
              value))

(defn valid-jon-gui-data-compass-calibration-status?
  "Validate status field of JonGuiDataCompassCalibration - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :status)
                               (drop 1 jon-gui-data-compass-calibration-spec))))
              value))

;; Validation helpers for JonGuiDataRotary
(defn valid-jon-gui-data-rotary-azimuth?
  "Validate azimuth field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :azimuth)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-azimuth-speed?
  "Validate azimuth-speed field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :azimuth-speed)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-elevation?
  "Validate elevation field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :elevation)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-elevation-speed?
  "Validate elevation-speed field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :elevation-speed)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-platform-azimuth?
  "Validate platform-azimuth field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :platform-azimuth)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-platform-elevation?
  "Validate platform-elevation field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :platform-elevation)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-platform-bank?
  "Validate platform-bank field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :platform-bank)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-mode?
  "Validate mode field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :mode)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-scan-target?
  "Validate scan-target field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :scan-target)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-scan-target-max?
  "Validate scan-target-max field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :scan-target-max)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-sun-azimuth?
  "Validate sun-azimuth field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :sun-azimuth)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

(defn valid-jon-gui-data-rotary-sun-elevation?
  "Validate sun-elevation field of JonGuiDataRotary - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :sun-elevation)
                               (drop 1 jon-gui-data-rotary-spec))))
              value))

;; Validation helpers for ScanNode
(defn valid-scan-node-index?
  "Validate index field of ScanNode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :index)
                               (drop 1 scan-node-spec))))
              value))

(defn valid-scan-node-day-zoom-table-value?
  "Validate day-zoom-table-value field of ScanNode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :day-zoom-table-value)
                               (drop 1 scan-node-spec))))
              value))

(defn valid-scan-node-heat-zoom-table-value?
  "Validate heat-zoom-table-value field of ScanNode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :heat-zoom-table-value)
                               (drop 1 scan-node-spec))))
              value))

(defn valid-scan-node-azimuth?
  "Validate azimuth field of ScanNode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :azimuth)
                               (drop 1 scan-node-spec))))
              value))

(defn valid-scan-node-elevation?
  "Validate elevation field of ScanNode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :elevation)
                               (drop 1 scan-node-spec))))
              value))

(defn valid-scan-node-linger?
  "Validate linger field of ScanNode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :linger)
                               (drop 1 scan-node-spec))))
              value))

(defn valid-scan-node-speed?
  "Validate speed field of ScanNode - has validation constraints"
  [value]
...
(defn valid-jon-gui-data-camera-day-focus-pos?
  "Validate focus-pos field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :focus-pos)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-zoom-pos?
  "Validate zoom-pos field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :zoom-pos)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-iris-pos?
  "Validate iris-pos field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :iris-pos)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-zoom-table-pos?
  "Validate zoom-table-pos field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :zoom-table-pos)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-zoom-table-pos-max?
  "Validate zoom-table-pos-max field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :zoom-table-pos-max)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-fx-mode?
  "Validate fx-mode field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :fx-mode)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-digital-zoom-level?
  "Validate digital-zoom-level field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :digital-zoom-level)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

(defn valid-jon-gui-data-camera-day-clahe-level?
  "Validate clahe-level field of JonGuiDataCameraDay - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :clahe-level)
                               (drop 1 jon-gui-data-camera-day-spec))))
              value))

;; Validation helpers for JonGuiDataCameraHeat
(defn valid-jon-gui-data-camera-heat-zoom-pos?
  "Validate zoom-pos field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :zoom-pos)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-agc-mode?
  "Validate agc-mode field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :agc-mode)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-filter?
  "Validate filter field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :filter)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-zoom-table-pos?
  "Validate zoom-table-pos field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :zoom-table-pos)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-zoom-table-pos-max?
  "Validate zoom-table-pos-max field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :zoom-table-pos-max)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-dde-level?
  "Validate dde-level field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :dde-level)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-fx-mode?
  "Validate fx-mode field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :fx-mode)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-digital-zoom-level?
  "Validate digital-zoom-level field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :digital-zoom-level)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))

(defn valid-jon-gui-data-camera-heat-clahe-level?
  "Validate clahe-level field of JonGuiDataCameraHeat - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :clahe-level)
                               (drop 1 jon-gui-data-camera-heat-spec))))
              value))
...
(defn valid-jon-gui-data-rec-osd-screen?
  "Validate screen field of JonGuiDataRecOsd - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :screen)
                               (drop 1 jon-gui-data-rec-osd-spec))))
              value))

;; Validation helpers for JonGuiDataDayCamGlassHeater
(defn valid-jon-gui-data-day-cam-glass-heater-temperature?
  "Validate temperature field of JonGuiDataDayCamGlassHeater - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :temperature)
                               (drop 1
                                     jon-gui-data-day-cam-glass-heater-spec))))
              value))

;; Validation helpers for JonGuiDataActualSpaceTime
(defn valid-jon-gui-data-actual-space-time-azimuth?
  "Validate azimuth field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :azimuth)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))

(defn valid-jon-gui-data-actual-space-time-elevation?
  "Validate elevation field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :elevation)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))

(defn valid-jon-gui-data-actual-space-time-bank?
  "Validate bank field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :bank)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))

(defn valid-jon-gui-data-actual-space-time-latitude?
  "Validate latitude field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :latitude)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))

(defn valid-jon-gui-data-actual-space-time-longitude?
  "Validate longitude field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :longitude)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))

(defn valid-jon-gui-data-actual-space-time-altitude?
  "Validate altitude field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :altitude)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))

(defn valid-jon-gui-data-actual-space-time-timestamp?
  "Validate timestamp field of JonGuiDataActualSpaceTime - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :timestamp)
                               (drop 1 jon-gui-data-actual-space-time-spec))))
              value))
...
(defn valid-jon-gui-state-protocol-version?
  "Validate protocol-version field of JonGUIState - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :protocol-version)
                               (drop 1 jon-gui-state-spec))))
              value))