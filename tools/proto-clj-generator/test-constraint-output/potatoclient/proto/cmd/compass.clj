...
(>defn build-calibrate-start-short
       ...
       [m]
       [calibrate-start-short-spec => any?]
       (let [builder
...
(>defn build-set-magnetic-declination
       ...
       [m]
       [set-magnetic-declination-spec => any?]
       (let
         [builder
...
(>defn build-set-offset-angle-azimuth
       ...
       [m]
       [set-offset-angle-azimuth-spec => any?]
       (let
         [builder
...
(>defn
  ...
  [m]
  [set-offset-angle-elevation-spec => any?]
  (let [builder
...
(>defn build-set-use-rotary-position
       ...
       [m]
       [set-use-rotary-position-spec => any?]
       (let
         [builder
...
(>defn parse-set-magnetic-declination
       ...
       [any? => set-magnetic-declination-spec]
       (cond-> {}
         ;; Regular fields
...
(>defn parse-set-offset-angle-azimuth
       ...
       [any? => set-offset-angle-azimuth-spec]
       (cond-> {}
         ;; Regular fields
         true (assoc :value (.getValue proto))))
...
(>defn parse-set-offset-angle-elevation
       ...
       [any? => set-offset-angle-elevation-spec]
       (cond-> {}
         ;; Regular fields
         true (assoc :value (.getValue proto))))
...
(>defn parse-set-use-rotary-position
       ...
       [any? => set-use-rotary-position-spec]
       (cond-> {}
         ;; Regular fields
...
;; Validation helpers for SetMagneticDeclination
(defn valid-set-magnetic-declination-value?
  "Validate value field of SetMagneticDeclination - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-magnetic-declination-spec))))
              value))

;; Validation helpers for SetOffsetAngleAzimuth
(defn valid-set-offset-angle-azimuth-value?
  "Validate value field of SetOffsetAngleAzimuth - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-offset-angle-azimuth-spec))))
              value))

;; Validation helpers for SetOffsetAngleElevation
(defn valid-set-offset-angle-elevation-value?
  "Validate value field of SetOffsetAngleElevation - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-offset-angle-elevation-spec))))
              value))