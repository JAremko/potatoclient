...
(defn valid-set-manual-position-latitude?
  "Validate latitude field of SetManualPosition - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :latitude)
                               (drop 1 set-manual-position-spec))))
              value))

(defn valid-set-manual-position-longitude?
  "Validate longitude field of SetManualPosition - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :longitude)
                               (drop 1 set-manual-position-spec))))
              value))

(defn valid-set-manual-position-altitude?
  "Validate altitude field of SetManualPosition - has validation constraints"
  [value]
