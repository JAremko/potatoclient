...
(defn valid-set-offsets-x?
  "Validate x field of SetOffsets - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :x)
                               (drop 1 set-offsets-spec))))
              value))

(defn valid-set-offsets-y?
  "Validate y field of SetOffsets - has validation constraints"
  [value]
...
(defn valid-shift-offsets-by-x?
  "Validate x field of ShiftOffsetsBy - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :x)
                               (drop 1 shift-offsets-by-spec))))
              value))

(defn valid-shift-offsets-by-y?
  "Validate y field of ShiftOffsetsBy - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :y)
                               (drop 1 shift-offsets-by-spec))))
              value))