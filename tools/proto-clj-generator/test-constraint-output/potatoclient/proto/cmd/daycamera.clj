...
(defn valid-move-target-value?
  "Validate target-value field of Move - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-value)
                               (drop 1 move-spec))))
              value))

(defn valid-move-speed?
  "Validate speed field of Move - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :speed) (drop 1 move-spec))))
              value))

;; Validation helpers for Offset
(defn valid-offset-offset-value?
  "Validate offset-value field of Offset - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :offset-value)
                               (drop 1 offset-spec))))
              value))
...
(defn valid-set-iris-value?
  "Validate value field of SetIris - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-iris-spec))))
              value))
