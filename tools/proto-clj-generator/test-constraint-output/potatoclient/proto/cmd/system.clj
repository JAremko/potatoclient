...
;; Validation helpers for SetLocalization
(defn valid-set-localization-loc?
  "Validate loc field of SetLocalization - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :loc)
                               (drop 1 set-localization-spec))))
              value))