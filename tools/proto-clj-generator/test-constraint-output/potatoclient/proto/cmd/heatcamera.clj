...
(defn valid-set-fx-mode-mode?
  "Validate mode field of SetFxMode - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :mode)
                               (drop 1 set-fx-mode-spec))))
              value))

;; Validation helpers for SetClaheLevel
(defn valid-set-clahe-level-value?
  "Validate value field of SetClaheLevel - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-clahe-level-spec))))
              value))

;; Validation helpers for ShiftClaheLevel
(defn valid-shift-clahe-level-value?
  "Validate value field of ShiftClaheLevel - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 shift-clahe-level-spec))))
              value))

;; Validation helpers for SetValue
(defn valid-set-value-value?
  "Validate value field of SetValue - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-value-spec))))
              value))

;; Validation helpers for SetDDELevel
(defn valid-set-dde-level-value?
  "Validate value field of SetDDELevel - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-dde-level-spec))))
              value))

;; Validation helpers for SetDigitalZoomLevel
(defn valid-set-digital-zoom-level-value?
  "Validate value field of SetDigitalZoomLevel - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-digital-zoom-level-spec))))
              value))

;; Validation helpers for ShiftDDE
(defn valid-shift-dde-value?
  "Validate value field of ShiftDDE - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 shift-dde-spec))))
              value))

;; Validation helpers for SetZoomTableValue
(defn valid-set-zoom-table-value-value?
  "Validate value field of SetZoomTableValue - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-zoom-table-value-spec))))
              value))

;; Validation helpers for SetAGC
(defn valid-set-agc-value?
  "Validate value field of SetAGC - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-agc-spec))))
              value))

;; Validation helpers for SetFilters
(defn valid-set-filters-value?
  "Validate value field of SetFilters - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :value)
                               (drop 1 set-filters-spec))))
              value))