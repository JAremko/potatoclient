(ns potatoclient.proto.cmd.lira
...
;; Validation helpers for JonGuiDataLiraTarget
(defn valid-jon-gui-data-lira-target-timestamp?
  "Validate timestamp field of JonGuiDataLiraTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :timestamp)
                               (drop 1 jon-gui-data-lira-target-spec))))
              value))

(defn valid-jon-gui-data-lira-target-target-longitude?
  "Validate target-longitude field of JonGuiDataLiraTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-longitude)
                               (drop 1 jon-gui-data-lira-target-spec))))
              value))

(defn valid-jon-gui-data-lira-target-target-latitude?
  "Validate target-latitude field of JonGuiDataLiraTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-latitude)
                               (drop 1 jon-gui-data-lira-target-spec))))
              value))

(defn valid-jon-gui-data-lira-target-target-azimuth?
  "Validate target-azimuth field of JonGuiDataLiraTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-azimuth)
                               (drop 1 jon-gui-data-lira-target-spec))))
              value))

(defn valid-jon-gui-data-lira-target-target-elevation?
  "Validate target-elevation field of JonGuiDataLiraTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :target-elevation)
                               (drop 1 jon-gui-data-lira-target-spec))))
              value))

(defn valid-jon-gui-data-lira-target-distance?
  "Validate distance field of JonGuiDataLiraTarget - has validation constraints"
  [value]
  (m/validate (second (first (filter #(= (first %) :distance)
                               (drop 1 jon-gui-data-lira-target-spec))))
              value))