...
(defn- create-styled-label
  ...
  {:malli/schema [:=> [:cat :string [:? :map]] [:fn (partial instance? JLabel)]]}
...
(defn- create-info-panel
  ...
  {:malli/schema [:=> [:cat] [:fn (partial instance? JPanel)]]}
  []
  (let [version (try
...
(defn- create-button-panel
  ...
  {:malli/schema [:=> [:cat [:fn (partial instance? JDialog)]] [:fn (partial instance? JPanel)]]}
  [dialog]
