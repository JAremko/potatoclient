...
(deftest test-error-handling
  (testing "Error handling functionality"
    ...
    (let [test-fn (fn [] (throw (Exception. "Test error")))]
      ...
      (is (some? @status-bar/last-error-atom)
          "Last error should be stored")
      (is (= "Test error" (:message @status-bar/last-error-atom))
          "Error message should be stored")
      (is (string? (:stack-trace @status-bar/last-error-atom))
    ...
    (let [result (status-bar/with-error-handler (fn [] "success"))]
      ...
      (is (nil? @status-bar/last-error-atom) "No error should be stored on success"))))
