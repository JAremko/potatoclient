...
(deftest test-error-handler-nil-exception
  (testing "Error handler with nil or empty exceptions"
    (state/reset-state!)
    (reset! status-bar/last-error-atom nil)
    ...
    (is (some? @status-bar/last-error-atom) "Error should still be stored")
    (is (= "" (:message @status-bar/last-error-atom)) "Stored message should be empty")
    (is (string? (:stack-trace @status-bar/last-error-atom)) "Stack trace should exist")))
...
(deftest test-sanity-error-recovery
  (testing "Sanity check: system recovers from errors"
    ...
    (is (some? @status-bar/last-error-atom) "Error should be captured")
    ...
    ;; Can clear error
    (reset! status-bar/last-error-atom nil)
    (is (nil? @status-bar/last-error-atom))))