(ns potatoclient.proto-constraints-test
  "Tests to document that buf.validate constraints are present in proto files
   but not enforced at runtime with standard bindings."
...
(deftest test-proto-serialization-works
  (testing "Proto serialization works regardless of constraint values"
    (testing "Can serialize and deserialize messages with any values"
      (let [cmd {:protocol-version 0 ; Would violate constraint
                 :session-id 12345
                 :important false
        ...
        (is (bytes? serialized)
            "Can serialize command with constraint-violating values")))))