...
(defn get-current-theme
  ...
  {:malli/schema [:=> [:cat] :potatoclient.ui-specs/theme-key]}
  []
...
(defn get-theme-i18n-key
  ...
  {:malli/schema [:=> [:cat :potatoclient.ui-specs/theme-key] :keyword]}
  [theme-key]
  (case theme-key
...
(defn get-available-themes
  ...
  {:malli/schema [:=> [:cat] [:sequential :potatoclient.ui-specs/theme-key]]}
  []
