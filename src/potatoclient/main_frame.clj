...
(defn- create-main-content
  "Create the main content panel." {:malli/schema [:=> [:cat] [:fn {:error/message "must be a Swing panel"} (fn* [p1__3842#] (instance? JPanel p1__3842#))]]}
  []
  ;; Since we removed the log view, just show the control panel
  (control-panel/create))

(defn- add-window-close-handler!
  "Add window close handler for proper cleanup." {:malli/schema [:=> [:cat [:fn {:error/message "must be a JFrame"} (fn* [p1__3844#] (instance? JFrame p1__3844#))]] :nil]}
  [frame]
  (seesaw/listen frame
                 :window-closing
                 (fn [_]
...
(defn create-main-frame
  ...
  IMPORTANT: This should be called on the Event Dispatch Thread." {:malli/schema [:=> [:cat :map] [:fn {:error/message "must be a JFrame"} (fn* [p1__3846#] (instance? JFrame p1__3846#))]]}
  [params]
