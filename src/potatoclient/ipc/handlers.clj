...
(defn process-queue
  ...
  {:malli/schema [:=> [:cat [:map
                              [:queue [:fn #(instance? LinkedBlockingQueue %)]]
...
(defn create-composite-handler
  ...
  {:malli/schema [:=> [:cat [:vector [:fn #(satisfies? IMessageHandler %)]]
                             [:fn #(instance? clojure.lang.Atom %)]]
                       [:fn #(satisfies? IMessageHandler %)]]}
  [handlers running?]
...
(defn create-filtering-handler
  ...
  {:malli/schema [:=> [:cat [:fn #(satisfies? IMessageHandler %)]
                             [:=> [:cat :any] :boolean]
                             [:fn #(instance? clojure.lang.Atom %)]]
                       [:fn #(satisfies? IMessageHandler %)]]}
