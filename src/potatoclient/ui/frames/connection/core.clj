  ...
  (let [domain (state/get-domain)
        ...
        update-ui! (fn []
                    (let [elapsed (.getSeconds (Duration/between @start-time (Instant/now)))
                      ...
                      (seesaw/config! attempts-label :text (str (i18n/tr :connection-attempts) ": " @attempts))
                      (seesaw/config! time-label :text (str (i18n/tr :connection-time) ": " (format-duration elapsed)))))
    ...
    (reset! update-timer
            (timer/timer (fn [_] (update-ui!)) :delay 1000))
    ...
    ;; Timers start automatically when created with timer/timer
