(ns potatoclient.ui.frames.initial.core
  "Initial frame for server connection with theme and localization support."
  (:require
...
(defn- create-content-panel
  "Create the main content panel for the initial frame."
  {:malli/schema [:=> [:cat [:maybe string?]] [:fn {:error/message "must be a Swing panel"} (partial instance? JPanel)]]}
  [saved-url]
  (let [url-label (seesaw/label :text (i18n/tr :startup-server-url) :font {:size 14})
        recent-urls (config/get-recent-urls)
        combo-items (vec (distinct (concat (when saved-url [saved-url]) recent-urls)))
        url-combobox (seesaw/combobox
                       :id :url-input
                       :model combo-items
                       :editable? true
                       :font {:size 14}
    ...
    (mig/mig-panel
      :constraints ["wrap 1, insets 20, gap 10" "[grow, fill]" "[]"]
      :items [[url-label ""]
              [url-combobox "growx, h 40!"]
              [(seesaw/separator) "growx, gaptop 10, gapbottom 10"]])))

(defn show
  "Show the initial frame and call the callback with :connect, :cancel, or :reload."
