...
(defn throttle
  ...
  {:malli/schema [:=> [:cat :ifn :pos-int] :ifn]}
  [f period-ms]
...
(defn with-busy-cursor
  ...
  {:malli/schema [:=> [:cat [:fn {:error/message "must be a Swing Component"} (partial instance? Component)] :ifn] :any]}
  [component f]
...
(defn preserve-selection
  ...
  {:malli/schema [:=> [:cat [:fn {:error/message "must be a text component"} (partial instance? JTextComponent)] :ifn] :nil]}
