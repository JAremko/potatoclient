...
(defn- reload-dialog!
  "Reload the dialog with new theme/locale." {:malli/schema [:=> [:cat [:fn {:error/message "must be a JFrame"} (fn* [p1__3852#] (instance? JFrame p1__3852#))] :ifn] :nil]}
  [dialog callback]
...
(defn- create-content-panel
  "Create the main content panel for the startup dialog." {:malli/schema [:=> [:cat [:maybe :string]] [:fn {:error/message "must be a Swing panel"} (fn* [p1__3854#] (instance? JPanel p1__3854#))]]}
  [saved-url]
...
(defn show-startup-dialog
  "Show the startup dialog and call the callback with :connect, :cancel, or :reload." {:malli/schema [:=> [:cat [:maybe [:fn {:error/message "must be a JFrame"} (fn* [p1__3856#] (instance? JFrame p1__3856#))]] :ifn] :nil]}
  [parent callback]
