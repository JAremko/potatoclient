...
class MessageProtocol(
    ...
    private val responseTemplate =
        ConcurrentHashMap<String, Any>(Constants.MAP_INITIAL_CAPACITY).apply {
            put("type", "response")
    ...
    private val navTemplate =
        ConcurrentHashMap<String, Any>(Constants.MAP_INITIAL_CAPACITY).apply {
            put("type", "navigation")
            put("streamId", streamId)
        }

    private val windowTemplate =
        ConcurrentHashMap<String, Any>(Constants.MAP_INITIAL_CAPACITY).apply {
            put("type", "window")
            put("streamId", streamId)
        }
    ...
    fun sendResponse(
        ...
        try {
            val resp =
                HashMap(responseTemplate).apply {
                    put("status", status)
                    put("data", data)
    ...
    private val threadLocalEventMap =
    ...
    fun sendWindowEvent(
        ...
        try {
            val event =
                HashMap<String, Any>(2 + (details?.size ?: 0)).apply {
                    put(KEY_TYPE, eventType)
                    details?.let { putAll(it) }
                }

            val window =
                HashMap(windowTemplate).apply {
                    put(KEY_EVENT, event)
                    put(KEY_TIMESTAMP, System.currentTimeMillis())
                }
