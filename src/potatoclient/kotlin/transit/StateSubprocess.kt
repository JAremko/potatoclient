    ...
    private suspend fun handleProtobufState(protoBytes: ByteArray) {
        ...
        try {
            ...
            transitComm.sendMessage(
                transitComm.createMessage("state", transitState)
            )
 ...
 */
class RateLimiter(val rateHz: Int) {
...
/**
 * Converts protobuf state to Transit-friendly maps
 */
class ProtobufToTransitConverter {
    fun convert(proto: JonSharedData.JonGUIState): Map<String, Any?> {
        return mapOf(
            ...
            "day-camera" to convertDayCamera(proto.dayCamera),
            "heat-camera" to convertHeatCamera(proto.heatCamera)
        )
    }
    
    private fun convertSystem(system: JonSharedData.System?): Map<String, Any?>? {
    ...
    private fun convertGps(gps: JonSharedData.Gps?): Map<String, Any?>? {
        ...
        return mapOf(
            ...
            "hdop" to gps.hdop,
            "mode" to gps.mode.name.lowercase()
        )
    }
    
    private fun convertCompass(compass: JonSharedData.Compass?): Map<String, Any?>? {
    ...
    private fun convertLrf(lrf: JonSharedData.Lrf?): Map<String, Any?>? {
        if (lrf == null) return null
        return mapOf(
    ...
    private fun convertRotary(rotary: JonSharedData.Rotary?): Map<String, Any?>? {
    ...
    private fun convertDayCamera(camera: JonSharedData.DayCamera?): Map<String, Any?>? {
        if (camera == null) return null
        return mapOf(
            "zoom" to camera.zoom,
            "focus-mode" to camera.focusMode.name.lowercase(),
            "focus-distance" to camera.focusDistance
        )
    }
    
    private fun convertHeatCamera(camera: JonSharedData.HeatCamera?): Map<String, Any?>? {
        if (camera == null) return null
        return mapOf(
            "zoom" to camera.zoom,
            "palette" to camera.palette.name.lowercase()
        )
    }
}
 ...
 */
class StateWebSocketClient(private val url: String) {
...
// Extension function
suspend fun <T> CompletableFuture<T>.await(): T = withContext(Dispatchers.IO) {
    this@await.get()
}
