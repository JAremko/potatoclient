...
(>defn toggle-osd
  "Toggle OSD on both cameras - if either is on, turn both off; otherwise turn both on.
   Returns a vector of two cmd roots: [day-cmd heat-cmd]."
  [day-enabled? heat-enabled?]
  [:boolean :boolean => [:vector :cmd/root]]
...
(>defn show-lrf-workflow
  "Show the complete LRF workflow screens in sequence.
   ...
   This is useful for demonstrating the LRF measurement process."
  []
  [=> [:vector :cmd/root]]
...
(>defn disable-all-osd
  "Disable OSD on both day and heat cameras.
   Returns a vector of two cmd roots: [day-disable-cmd heat-disable-cmd]."
  []
  [=> [:vector :cmd/root]]
  [(disable-day-osd)
   (disable-heat-osd)])
...
(>defn enable-all-osd
  "Enable OSD on both day and heat cameras.
   Returns a vector of two cmd roots: [day-enable-cmd heat-enable-cmd]."
  []
  [=> [:vector :cmd/root]]
  [(enable-day-osd)
   (enable-heat-osd)])