...
(defn set-agc
  "Set the AGC (Automatic Gain Control) mode.
   ...
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat :enum/heat-agc-mode] :cmd/root]}
  [mode]
  (core/create-command 
...
(defn set-filter
  "Set the heat camera filter (thermal visualization mode).
   ...
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat :enum/heat-filter] :cmd/root]}
  [filter]
  (core/create-command 
...
(defn set-digital-zoom-level
  "Set the digital zoom level (must be >= 1.0).
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat :range/digital-zoom] :cmd/root]}
  [value]
  (core/create-command 
...
(defn set-dde-level
  "Set the DDE level (0 to 100).
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat [:int {:min 0, :max 100}]] :cmd/root]}
  [level]
  (core/create-command 
...
(defn shift-dde
  "Shift the DDE level by offset (-100 to 100).
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat [:int {:min -100, :max 100}]] :cmd/root]}
  [shift-value]
  (core/create-command 
...
(defn set-fx-mode
  "Set the FX mode for the heat camera.
   ...
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat :enum/fx-mode-heat] :cmd/root]}
  [mode]
  (core/create-command 
...
(defn set-clahe-level
  "Set the CLAHE level (0.0 to 1.0).
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat :range/normalized] :cmd/root]}
  [value]
  (core/create-command 
...
(defn shift-clahe-level
  "Shift the CLAHE level by offset (-1.0 to 1.0).
   Returns a fully formed cmd root ready to send." {:malli/schema [:=> [:cat :range/normalized-offset] :cmd/root]}
  [shift-value]
  (core/create-command 
