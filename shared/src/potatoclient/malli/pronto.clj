...
(defn create-optimized-proto-map
  "Create a proto-map with optimal performance by passing all initial values
   directly to p/proto-map, avoiding multiple assoc operations.
   ...
   Best practice: Always prefer creating with initial values over multiple assocs."
  [mapper proto-class field-values]
  ;; Convert map to flat key-value sequence for proto-map macro
  ;; Since p/proto-map is a macro, we need to use eval (not ideal)
  ;; For production, consider generating code at compile time
  (if (empty? field-values)
    (p/proto-map mapper proto-class)
    ;; Fall back to merge for now - in production, consider code generation
    (merge (p/proto-map mapper proto-class) field-values)))

(defmacro batch-update-proto-map-macro
  "Macro version for compile-time optimization of batch updates.
   Generates optimal p/p-> code for known updates.
   
   Usage:
   (batch-update-proto-map-macro my-proto-map
     :field1 value1
     :field2 value2)"
  [proto-map & kvs]
  (assert (even? (count kvs)) "Must provide even number of key-value pairs")
  (let [pairs (partition 2 kvs)
        assoc-forms (map (fn [[k v]] `(~'assoc ~k ~v)) pairs)]
    `(p/p-> ~proto-map
            ~@assoc-forms)))

(defn batch-update-proto-map
  "Update multiple fields efficiently using p/p-> with transients.
   
   Parameters:
   - proto-map: The proto-map to update
   - updates: A map of field names to new values
   
   Uses p/p-> which automatically uses transients for multiple operations."
  [proto-map updates]
  (if (empty? updates)
    proto-map
    (reduce-kv
...
(defmacro with-hinted-proto-map
  "Macro for working with hinted proto-maps for maximum performance.
   
   Parameters:
   - proto-map-sym: Symbol bound to the proto-map
   - proto-class: The Java protobuf class
   - mapper: The Pronto mapper
   - body: Code to execute with hints
   
   Usage:
   (with-hinted-proto-map [my-map MyClass my-mapper]
     (p/p-> my-map
            (assoc :field1 value1)
            (assoc :field2 value2)))"
  [[proto-map-sym proto-class mapper] & body]
  `(p/with-hints
     [(p/hint ~proto-map-sym ~proto-class ~mapper)]
     ~@body))

(defmacro optimized-assoc
  "Optimized assoc for proto-maps when class is known at compile time.
   Uses p/p-> with hints for best performance.
   
   Usage:
   (optimized-assoc my-proto MyClass my-mapper
     :field1 value1
     :field2 value2)"
  [proto-map proto-class mapper & kvs]
  (assert (even? (count kvs)) "Must provide even number of key-value pairs")
  (let [pairs (partition 2 kvs)
        assoc-forms (map (fn [[k v]] `(~'assoc ~k ~v)) pairs)]
    `(p/p-> (p/hint ~proto-map ~proto-class ~mapper)
            ~@assoc-forms)))

(defmacro optimized-get
  "Optimized get for proto-maps when class is known at compile time.
   Uses p/p-> with hints to generate direct getter calls.
   
   Usage:
   (optimized-get my-proto MyClass my-mapper :field-name)"
  [proto-map proto-class mapper field]
  `(p/p-> (p/hint ~proto-map ~proto-class ~mapper) ~field))

(defmacro optimized-get-in
  "Optimized get-in for nested proto-maps with hints.
   
   Usage:
   (optimized-get-in my-proto MyClass my-mapper [:address :city])"
  [proto-map proto-class mapper path]
  (let [field-accesses (map (fn [f] f) path)]
    `(p/p-> (p/hint ~proto-map ~proto-class ~mapper)
            ~@field-accesses)))

;; ============================================================================
;; Performance Best Practices (from Pronto docs)
;; ============================================================================

(defn create-with-values
  "Create a proto-map with initial values - FASTEST approach.
   
   DO THIS:
   (create-with-values mapper Person {:name \"joe\" :age 99})
   
   DON'T DO:
   (-> (p/proto-map mapper Person) (assoc :name \"joe\") (assoc :age 99))"
  [mapper proto-class values-map]
  ;; In production, generate this at compile time for known schemas
  (if (empty? values-map)
    (p/proto-map mapper proto-class)
    ;; This is the fallback - ideally use code generation
    (merge (p/proto-map mapper proto-class) values-map)))

(defn update-multiple-fields
  "Update multiple fields efficiently using transients via p/p->.
   
   Parameters:
   - proto-map: The proto-map to update
   - updates: Vector of [operation field value] tuples
   
   Example:
   (update-multiple-fields my-proto
     [[:assoc :name \"Alice\"]
      [:update :age inc]
      [:assoc-in [:address :city] \"NYC\"]])"
  [proto-map updates]
  (if (empty? updates)
    proto-map
    (eval
...
(defn benchmark-proto-operations
  "Benchmark various proto-map operations for performance analysis.
   
   Returns timing information for different operation types."
  [mapper proto-class iterations]
  (let [empty-proto (p/proto-map mapper proto-class)
        
        ;; Benchmark single assoc
        single-assoc-time 
        (let [start (System/nanoTime)]
          (dotimes [_ iterations]
            (assoc empty-proto :field1 "value"))
          (- (System/nanoTime) start))
        
        ;; Benchmark p/p-> with multiple assocs
        multi-assoc-time
        (let [start (System/nanoTime)]
          (dotimes [_ iterations]
            (p/p-> empty-proto
        ...
        ;; Benchmark create with values
        create-time
        (let [start (System/nanoTime)]
          (dotimes [_ iterations]
            (create-with-values mapper proto-class
                              {:field1 "value1"
                               :field2 "value2"
                               :field3 "value3"}))
          (- (System/nanoTime) start))]
    
    {:single-assoc-ns (/ single-assoc-time iterations)
     :multi-assoc-ns (/ multi-assoc-time iterations)
     :create-with-values-ns (/ create-time iterations)
     :speedup-ratio (double (/ single-assoc-time create-time))}))