...
(def jon-gui-data-lira-target "Schema for jon-gui-data-lira-target" [:map [:timestamp [:and [:maybe :int] [:>= "0"]]] [:target-longitude [:maybe :double]] [:target-latitude [:maybe :double]] [:target-altitude [:maybe :double]] [:target-azimuth [:maybe :double]] [:target-elevation [:maybe :double]] [:distance [:maybe :double]] [:uuid-part1 [:maybe :int]] [:uuid-part2 [:maybe :int]] [:uuid-part3 [:maybe :int]] [:uuid-part4 [:maybe :int]]])