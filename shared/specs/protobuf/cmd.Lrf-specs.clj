...
(def root "Schema for root" [:map [:cmd [:oneof {:target-designator-off [:map [:target-designator-off :cmd/target-designator-off]], :target-designator-on-mode-b [:map [:target-designator-on-mode-b :cmd/target-designator-on-mode-b]], :disable-fog-mode [:map [:disable-fog-mode :cmd/disable-fog-mode]], :set-scan-mode [:map [:set-scan-mode :cmd/set-scan-mode]], :refine-off [:map [:refine-off :cmd/refine-off]], :scan-off [:map [:scan-off :cmd/scan-off]], :refine-on [:map [:refine-on :cmd/refine-on]], :start [:map [:start :cmd/start]], :measure [:map [:measure :cmd/measure]], :scan-on [:map [:scan-on :cmd/scan-on]], :stop [:map [:stop :cmd/stop]], :new-session [:map [:new-session :cmd/new-session]], :get-meteo [:map [:get-meteo :cmd/get-meteo]], :enable-fog-mode [:map [:enable-fog-mode :cmd/enable-fog-mode]], :target-designator-on-mode-a [:map [:target-designator-on-mode-a :cmd/target-designator-on-mode-a]]}]]])
