...
(def elevation "Schema for elevation" [:map [:cmd [:oneof {:set-value [:map [:set-value :cmd/set-elevation-value]], :rotate-to [:map [:rotate-to :cmd/rotate-elevation-to]], :rotate [:map [:rotate :cmd/rotate-elevation]], :relative [:map [:relative :cmd/rotate-elevation-relative]], :relative-set [:map [:relative-set :cmd/rotate-elevation-relative-set]], :halt [:map [:halt :cmd/halt-elevation]]}]]])
...
(def azimuth "Schema for azimuth" [:map [:cmd [:oneof {:set-value [:map [:set-value :cmd/set-azimuth-value]], :rotate-to [:map [:rotate-to :cmd/rotate-azimuth-to]], :rotate [:map [:rotate :cmd/rotate-azimuth]], :relative [:map [:relative :cmd/rotate-azimuth-relative]], :relative-set [:map [:relative-set :cmd/rotate-azimuth-relative-set]], :halt [:map [:halt :cmd/halt-azimuth]]}]]])
...
(def root "Schema for root" [:map [:cmd [:oneof {:rotate-to-gps [:map [:rotate-to-gps :cmd/rotate-to-gps]], :scan-pause [:map [:scan-pause :cmd/scan-pause]], :rotate-to-ndc [:map [:rotate-to-ndc :cmd/rotate-to-ndc]], :scan-start [:map [:scan-start :cmd/scan-start]], :set-platform-azimuth [:map [:set-platform-azimuth :cmd/set-platform-azimuth]], :scan-stop [:map [:scan-stop :cmd/scan-stop]], :start [:map [:start :cmd/start]], :stop [:map [:stop :cmd/stop]], :set-origin-gps [:map [:set-origin-gps :cmd/set-origin-gps]], :scan-next [:map [:scan-next :cmd/scan-next]], :set-platform-bank [:map [:set-platform-bank :cmd/set-platform-bank]], :get-meteo [:map [:get-meteo :cmd/get-meteo]], :set-use-rotary-as-compass [:map [:set-use-rotary-as-compass :cmd/set-use-rotary-as-compass]], :scan-prev [:map [:scan-prev :cmd/scan-prev]], :scan-add-node [:map [:scan-add-node :cmd/scan-add-node]], :set-platform-elevation [:map [:set-platform-elevation :cmd/set-platform-elevation]], :scan-select-node [:map [:scan-select-node :cmd/scan-select-node]], :halt [:map [:halt :cmd/halt]], :scan-delete-node [:map [:scan-delete-node :cmd/scan-delete-node]], :axis [:map [:axis :cmd/axis]], :scan-unpause [:map [:scan-unpause :cmd/scan-unpause]], :set-mode [:map [:set-mode :cmd/set-mode]], :scan-refresh-node-list [:map [:scan-refresh-node-list :cmd/scan-refresh-node-list]], :scan-update-node [:map [:scan-update-node :cmd/scan-update-node]]}]]])
