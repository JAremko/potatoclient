...
(def root "Schema for root" [:map [:cmd [:oneof {:start [:map [:start :cmd/start]], :stop [:map [:stop :cmd/stop]], :turn-on [:map [:turn-on :cmd/turn-on]], :turn-off [:map [:turn-off :cmd/turn-off]], :get-meteo [:map [:get-meteo :cmd/get-meteo]]}]]])
