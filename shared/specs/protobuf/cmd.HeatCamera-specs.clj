...
(def zoom "Schema for zoom" [:map [:cmd [:oneof {:set-zoom-table-value [:map [:set-zoom-table-value :cmd/set-zoom-table-value]], :next-zoom-table-pos [:map [:next-zoom-table-pos :cmd/next-zoom-table-pos]], :prev-zoom-table-pos [:map [:prev-zoom-table-pos :cmd/prev-zoom-table-pos]]}]]])
...
(def root "Schema for root" [:map [:cmd [:oneof {:set-dde-level [:map [:set-dde-level :cmd/set-dde-level]], :set-calib-mode [:map [:set-calib-mode :cmd/set-calib-mode]], :zoom [:map [:zoom :cmd/zoom]], :set-agc [:map [:set-agc :cmd/set-agc]], :shift-dde [:map [:shift-dde :cmd/shift-dde]], :set-filter [:map [:set-filter :cmd/set-filters]], :set-clahe-level [:map [:set-clahe-level :cmd/set-clahe-level]], :disable-dde [:map [:disable-dde :cmd/disable-dde]], :prev-fx-mode [:map [:prev-fx-mode :cmd/prev-fx-mode]], :start [:map [:start :cmd/start]], :focus-step-minus [:map [:focus-step-minus :cmd/focus-step-minus]], :set-digital-zoom-level [:map [:set-digital-zoom-level :cmd/set-digital-zoom-level]], :enable-dde [:map [:enable-dde :cmd/enable-dde]], :focus-stop [:map [:focus-stop :cmd/focus-stop]], :stop [:map [:stop :cmd/stop]], :reset-zoom [:map [:reset-zoom :cmd/reset-zoom]], :zoom-out [:map [:zoom-out :cmd/zoom-out]], :photo [:map [:photo :cmd/photo]], :zoom-in [:map [:zoom-in :cmd/zoom-in]], :get-meteo [:map [:get-meteo :cmd/get-meteo]], :focus-step-plus [:map [:focus-step-plus :cmd/focus-step-plus]], :set-fx-mode [:map [:set-fx-mode :cmd/set-fx-mode]], :refresh-fx-mode [:map [:refresh-fx-mode :cmd/refresh-fx-mode]], :focus-out [:map [:focus-out :cmd/focus-out]], :set-auto-focus [:map [:set-auto-focus :cmd/set-auto-focus]], :zoom-stop [:map [:zoom-stop :cmd/zoom-stop]], :save-to-table [:map [:save-to-table :cmd/save-to-table]], :next-fx-mode [:map [:next-fx-mode :cmd/next-fx-mode]], :calibrate [:map [:calibrate :cmd/calibrate]], :shift-clahe-level [:map [:shift-clahe-level :cmd/shift-clahe-level]], :focus-in [:map [:focus-in :cmd/focus-in]]}]]])
