...
(def offsets "Schema for offsets" [:map [:cmd [:oneof {:set [:map [:set :cmd/set-offsets]], :save [:map [:save :cmd/save-offsets]], :reset [:map [:reset :cmd/reset-offsets]], :shift [:map [:shift :cmd/shift-offsets-by]]}]]])
