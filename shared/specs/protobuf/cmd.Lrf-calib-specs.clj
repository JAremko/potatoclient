...
(def offsets "Schema for offsets" [:map [:cmd [:oneof {:set [:map [:set [:maybe :cmd/set-offsets]]], :save [:map [:save [:maybe :cmd/save-offsets]]], :reset [:map [:reset [:maybe :cmd/reset-offsets]]], :shift [:map [:shift [:maybe :cmd/shift-offsets-by]]]}]]])

(def set-offsets "Schema for set-offsets" [:map [:x [:and [:maybe :int] [:>= -1920] [:<= 1920]]] [:y [:and [:maybe :int] [:>= -1080] [:<= 1080]]]])

(def shift-offsets-by "Schema for shift-offsets-by" [:map [:x [:and [:maybe :int] [:>= -1920] [:<= 1920]]] [:y [:and [:maybe :int] [:>= -1080] [:<= 1080]]]])
