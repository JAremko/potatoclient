 ...
 #:malli{:schema
         [:=>
          ...
          [:fn
           ...
           (fn* [p1__297#] (instance? JPanel p1__297#))]]}
 []
         ...
         [:=>
          [:cat
           [:fn
            ...
            (fn* [p1__298#] (instance? JFrame p1__298#))]]
          :nil]}
 [frame]
 (seesaw/listen
  frame
  ...
  (fn
   ...
   (future
    (try
     ...
     (let
      [stream-processes
       (state/get-all-stream-processes)
       _
       (reduce-kv
        (fn
         [m k v]
         (let
          [stream-key (case k :heat-video :heat :day-video :day k)]
     ...
     (catch
      ...
      e
      (logging/log-error
       {:msg (str "Error during shutdown: " (.getMessage e))}))
...
(defn
 ...
 #:malli{:schema
         [:=>
          ...
          [:fn
           #:error{:message "must be a JFrame"}
           (fn* [p1__299#] (instance? JFrame p1__299#))]]}
 [params]
 (let
  [{:keys [version build-type window-state]}
   params
   _
   (theme/preload-theme-icons!)
   frame-cons
   (fn [state] (create-main-frame (assoc params :window-state state)))
   domain
   (config/get-domain)
   title
   (format
