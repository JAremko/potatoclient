...
(defn- extract-domain*
  ...
  {:malli/schema [:=> [:cat :string] :potatoclient.ui-specs/domain]}
  [input]
...
(defn- get-config-dir
  ...
  {:malli/schema [:=> [:cat] :potatoclient.ui-specs/file]}
  []
...
(defn load-config
  ...
  {:malli/schema [:=> [:cat] :potatoclient.ui-specs/config]}
  []
...
(defn save-config!
  ...
  {:malli/schema [:=> [:cat :potatoclient.ui-specs/config] :boolean]}
  [config]
...
(defn save-theme!
  ...
  {:malli/schema [:=> [:cat :potatoclient.ui-specs/theme-key] :boolean]}
  [theme-key]
...
(defn update-config!
  ...
  {:malli/schema [:=> [:cat :potatoclient.ui-specs/config-key [:or :potatoclient.ui-specs/theme-key :potatoclient.ui-specs/domain :potatoclient.ui-specs/locale :potatoclient.ui-specs/url-history]] :boolean]}
  [key value]
  (let [config (load-config)]
...
(defn get-url-history
  ...
  {:malli/schema [:=> [:cat] :potatoclient.ui-specs/url-history]}
  []
  (let [config (load-config)]
...
(defn get-domain
  ...
  {:malli/schema [:=> [:cat] :potatoclient.ui-specs/domain]}
  []
  (let [config (load-config)]
