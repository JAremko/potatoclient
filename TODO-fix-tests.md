...
### Tests to DELETE:
- [ ] `test/kotlin/CommandSubprocessTest.kt.skip` - Uses non-existent `SimpleCommandBuilder`
...
### Identify and Fix:
- [ ] Run `make test` and identify which tests hang
...
### Steps:
- [ ] Run `make test` and capture all failures
- [ ] Group failures by root cause
- [ ] Fix each group systematically
- [ ] No commenting out failing assertions!
- [ ] No reducing test coverage to make them pass!
...
1. Start with Phase 1 - Remove obsolete tests
2. Run `make test` to get baseline of current issues
3. Fix hanging tests first (they block everything else)
4. Then fix failing tests
5. Finally, rewrite disabled tests
6. Set up monitoring to prevent future test degradation
