  ...
  (let [stream-name (case stream-key
    ...
    (mig/mig-panel
      ...
      :items [[(seesaw/label :text stream-name)]
              ...
              [""]
              [info-label "span 2"]]))) 
 (m/=> create-stream-status-panel [:=> [:cat :potatoclient.ui-specs/stream-key] [:fn {:error/message "must be a Swing panel"} (partial instance? JPanel)]])
...
(defn- create-connection-info-panel
  ...
  (let [domain-label (seesaw/label :id :domain-info
    ...
    (seesaw/border-panel
      ...
      :center domain-label))) 
 (m/=> create-connection-info-panel [:=> [:cat] [:fn {:error/message "must be a Swing panel"} (partial instance? JPanel)]])
...
(defn- create-stream-controls-panel
 ...
 (m/=> create-stream-controls-panel [:=> [:cat] [:fn {:error/message "must be a Swing panel"} (partial instance? JPanel)]])
  ...
  (let [stats-label (seesaw/text :id :stats-info
    ...
    (seesaw/border-panel
      ...
      :center (seesaw/scrollable stats-label
                                 :border 0)))) 
 (m/=> create-statistics-panel [:=> [:cat] [:fn {:error/message "must be a Swing panel"} (partial instance? JPanel)]])
