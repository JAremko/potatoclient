...
(defn preserve-window-state
 ...
 (m/=> preserve-window-state [:=> [:cat [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)]] :potatoclient.ui-specs/window-state])
...
(defn restore-window-state!
 ...
 (m/=> restore-window-state! [:=> [:cat [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)] :potatoclient.ui-specs/window-state] [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)]])
...
(defn- reload-frame!
 ...
 (m/=> reload-frame! [:=> [:cat [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)] :ifn] :nil])
...
(defn- create-language-action
 ...
 (m/=> create-language-action [:=> [:cat :potatoclient.ui-specs/locale :string :ifn] :any])
      ...
      :handler (fn [e]
                 (when-not (= (theme/get-current-theme) theme-key)
                   (when (theme/set-theme! theme-key)
                     ...
                     (reload-frame! (seesaw/to-root e) reload-fn))))))) 
 (m/=> create-theme-action [:=> [:cat :potatoclient.ui-specs/theme-key :ifn] :any])
...
(defn- create-theme-menu
 ...
 (m/=> create-theme-menu [:=> [:cat :ifn] :any])
...
(defn- create-language-menu
 ...
 (m/=> create-language-menu [:=> [:cat :ifn] :any])
...
(defn- create-stream-toggle-button
  ...
  (let [stream-config {:heat {:endpoint "/ws/ws_rec_video_heat"
    ...
    button)) 
 (m/=> create-stream-toggle-button [:=> [:cat :potatoclient.ui-specs/stream-key] :any])
