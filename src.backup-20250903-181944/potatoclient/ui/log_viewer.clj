(ns potatoclient.ui.log-viewer
  ...
  (:require
            [malli.core :as m] [clojure.java.io]
...
(defn- get-log-directory
 ...
 (m/=> get-log-directory [:=> [:cat] [:fn {:error/message "must be a File"} (partial instance? File)]])
...
(defn- parse-log-filename
  ...
  (when-let [match (re-matches #"potatoclient-(.+?)-(\d{8}-\d{6})\.log" filename)]
    {:version (second match)
     ...
     :filename filename})) 
 (m/=> parse-log-filename [:=> [:cat :string] [:maybe [:map [:version :string] [:timestamp :string] [:filename :string]]]])
...
(defn- format-timestamp
  ...
  (try
    ...
    (catch Exception _
      timestamp-str))) 
 (m/=> format-timestamp [:=> [:cat :string] :string])
...
(defn- get-log-files
  ...
  (let [log-dir (get-log-directory)]
    (if (.exists log-dir)
      ...
      []))) 
 (m/=> get-log-files [:=> [:cat] [:sequential :map]])
...
(defn- format-file-size
 ...
 (m/=> format-file-size [:=> [:cat :int] :string])

(defn- copy-to-clipboard
  "Copy text to system clipboard."
  [text]
  (clipboard/contents! text)) 
 (m/=> copy-to-clipboard [:=> [:cat :string] :any])
...
(defn- dispose-frame!
  ...
  (let [frame-title (.getTitle frame)]
    ...
    (let [timer (Timer.
      ...
      (.start timer)))
  nil) 
 (m/=> dispose-frame! [:=> [:cat [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)]] :nil])
    ...
    (->> lines
         (map (fn [line]
                (if (re-matches #"^\d{4}-\d{2}-\d{2}T.*" line)
                  ...
                  line)))
         (str/join "\n")))) 
 (m/=> format-log-content [:=> [:cat :string] :string])
...
(defn- create-file-viewer
  ...
  (let [raw-content (slurp file)
    ...
    frame)) 
 (m/=> create-file-viewer [:=> [:cat [:fn {:error/message "must be a File"} (partial instance? File)] [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)]] [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)]])
...
(defn- pack-table-columns!
 ...
 (m/=> pack-table-columns! [:=> [:cat [:fn {:error/message "must be a JTable"} (partial instance? JTable)]] :nil])
...
(defn- create-log-table
  ...
  (seesaw/table
    ...
    :show-grid? true
    :auto-resize :off)) 
 (m/=> create-log-table [:=> [:cat] [:fn {:error/message "must be a JTable"} (partial instance? JTable)]])
...
(defn- update-log-list!
 ...
 (m/=> update-log-list! [:=> [:cat [:fn {:error/message "must be a JTable"} (partial instance? JTable)]] :nil])
...
(defn create-log-viewer-frame
  ...
  (let [table (create-log-table)
    ...
    frame)) 
 (m/=> create-log-viewer-frame [:=> [:cat] [:fn {:error/message "must be a JFrame"} (partial instance? JFrame)]])
