...
(defn- save-window-bounds!
  ...
  (let [bounds (.getBounds window)]
    (state/set-tab-window-bounds! tab-key
                                  ...
                                  (.height bounds)))
  nil) 
 (m/=> save-window-bounds! [:=> [:cat :keyword [:fn (fn* [p1__4189#] (instance? JFrame p1__4189#))]] :nil])
...
(defn close-detached-window!
  ...
  (when-let [window (get @detached-windows tab-key)]
    ...
    (status-bar/set-info! (str (i18n/tr :window-closed) " " (case tab-key
                                                               ...
                                                               (name tab-key)))))
  nil) 
 (m/=> close-detached-window! [:=> [:cat :keyword] :nil])
...
(defn create-detached-window!
  ...
  (let [;; Get binding group for this window
    ...
    window)) 
 (m/=> create-detached-window! [:=> [:cat :keyword [:=> [:cat [:maybe :keyword]] :any] [:fn (fn* [p1__4199#] (instance? JFrame p1__4199#))]] [:fn (fn* [p1__4201#] (instance? JFrame p1__4201#))]])
...
(defn toggle-detached-window!
  ...
  nil) 
 (m/=> toggle-detached-window! [:=> [:cat :keyword [:=> [:cat [:maybe :keyword]] :any] [:fn (fn* [p1__4207#] (instance? JFrame p1__4207#))]] :nil])

(defn get-detached-window
  "Get the detached window for a tab if it exists."
  [tab-key]
  (get @detached-windows tab-key)) 
 (m/=> get-detached-window [:=> [:cat :keyword] [:maybe [:fn (fn* [p1__4213#] (instance? JFrame p1__4213#))]]])
...
(defn reload-all-windows!
  ...
  nil) 
 (m/=> reload-all-windows! [:=> [:cat [:fn (fn* [p1__4219#] (instance? JFrame p1__4219#))] [:sequential :map]] :nil])